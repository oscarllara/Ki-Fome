...
const StoresPage = () => {
  ...
  const handleSaveStore = (e: React.FormEvent) => {
    ...
    } catch (e) { showError("Erro ao salvar loja."); }
  };
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-sm">
        <div className="p-6 border-b border-slate-50 bg-slate-50/30">
          <div className="relative w-full md:w-96">
            ...
            <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Buscar loja ou dono..." className="pl-10 h-12 bg-white rounded-xl border-slate-200" />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredStores.map((store) => (
                <tr key={store.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-8 py-4">
                    <Link to={`/admin/users/edit/${store.ownerId || 234}`} className="flex items-center gap-2 text-sm font-bold text-orange-600 hover:underline">
              ...
              <div className="space-y-12">
                ...
                <section className="space-y-6">
                  <div className="flex items-center gap-2 border-b pb-2">
                    ...
                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Contato & Redes</h3>
                  </div>
